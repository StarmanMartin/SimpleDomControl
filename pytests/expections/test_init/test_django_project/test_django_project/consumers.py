...
from test_django_project.sdc_extentions.response import sdc_link_factory, sdc_link_obj_factory
from test_django_project.sdc_extentions.import_manager import import_function
...
class SDCModelConsumer(WebsocketConsumer):
    ...
    def on_update(self, data):
        if data['args']['data'].pk in self.ids:
            data['args']['data'] = serializers.serialize('json', [data['args']['data']])
    ...
    def on_create(self, data):
        ...
        try:
            ...
            data['args']['data'] = serializers.serialize('json', [instance])
            self.send(text_data=json.dumps(data))
        except:
            pass
    ...
    def receive(self, text_data=None, bytes_data=None):
        ...
        try:
            ...
            if event_type == 'model_delete':
                ...
                model_data = serializers.serialize('json', self._load_model())
                self.send(text_data=json.dumps({
    ...
    def _load_create_form(self, json_data):
        return self._load_form(json_data, self.model.create_form)
    ...
    def _load_form(self, json_data, form_attr, instance=None):
        ...
        self.send(text_data=json.dumps({
            'type': json_data['event_type'],
            'event_id': json_data['event_id'],
            'html': self._render("elements/form.html", {'instance': instance, 'form': form(instance=instance)}, json_data),
            'is_error': False
        }))

    def _create_element(self, json_data):
        return self._submit_element(json_data, self.model.create_form)
    ...
    def _submit_element(self, json_data, form_attr, instance=None):
        ...
        if form_attr is None:
            ...
            form = form_attr
        form_instance = form(instance=instance, data=data)
        ...
        if is_valid:
            ...
            msg['header'] = 'Upss!'
            msg['msg'] = "<ul>%s</ul>" % "\n".join(["<li>%s: %s</li>" % (k, v[0]) for k, v in form_instance.errors.items()])

        self.send(text_data=json.dumps(msg | {
            'type': json_data['event_type'],
            'event_id': json_data['event_id'],
            'html': self._render("elements/form.html", {'instance': form_instance.instance, 'form': form_instance}, json_data),

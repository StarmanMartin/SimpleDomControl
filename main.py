...
from sdc_core_manager import add_sdc_core, clean_up, add_sdc_to_main_urls
import settings_manager
from add_controller_manager import AddControllerManager
from utils import copy, copy_and_prepare, makedirs_if_not_exist
import options
...
if __name__ == "__main__":
    ...
    if not os.path.exists(settings.get_settings_file_path()):
        print(options.CMD_COLORS.as_error(settings.get_settings_file_path() + " settingsfile does not exist!! sorry"))
        exit(1)
    ...
    if sys.argv[-1] == "init":
        main_static = os.path.join(options.PROJECT_ROOT, all_apps[0], "static")
        main_templates = os.path.join(options.PROJECT_ROOT, all_apps[0], "templates")
        ...
        settings.override_installed_apps()

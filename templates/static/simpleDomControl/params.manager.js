(function (app) {
    ...
    pm.runOnInitWithParameter = function (controller, $element) {
        runOnInitWithParameter(controller, $element, controller);
    };

    function runOnInitWithParameter(controller, $element, applyController) {
        if (!controller) {
            return false
        } else if (typeof controller.onInit !== 'function') {
            return runOnInitWithParameter(controller.super, $element, applyController);
        }

        var paramNameList = window.utils.getParamsNameOfFunction(controller.onInit);
        var paramList = pm.getParamList(paramNameList, $element);
        var initParams = pm.parse(controller.parentController, paramList);
        controller.onInit.apply(applyController, initParams);

        runOnInitWithParameter(controller.super, $element, applyController);
    }
    ...
    pm.getParamList = function (paramNameList, $element) {
        ...
        for (var i = 0; i < paramNameList.length; i++) {
            paramNameList[i] = app.domRouter.makeTag(paramNameList[i]);

            if ($element.attr('data-' + paramNameList[i])) {
                returnList.push($element.data(paramNameList[i]));
            } else {
